...
public class WindowBox extends DialogBox implements HasOpenHandlers<WindowBox>, 
	RequiresResize, HasResizeHandlers {
	 ...
	 * been added using {@link #add(com.google.gwt.user.client.ui.IsWidget)}. <br>
	 * <br>
	 ...
	 * @param draggable 
	 *            <code>true</code> to allow dragging the window
	 */
	public WindowBox(boolean resizeable, boolean draggable) {
		this(false, false, true, true, resizeable, draggable);
	}
	...
	public WindowBox(boolean showCloseIcon, boolean resizeable,
			boolean draggable) {
		this(false, false, true, showCloseIcon, resizeable, draggable);
	}
	...
	public WindowBox(boolean showMinimizeIcon, boolean showCloseIcon,
			boolean resizeable, boolean draggable) {
	...
	public WindowBox(boolean modal, boolean showMinimizeIcon,
			boolean showCloseIcon, boolean resizeable, boolean draggable) {
	...
	public void onBrowserEvent(Event event) {
		...
		if (this.resizable) {
			...
			case Event.ONMOUSEOUT:
				...
				if (this.dragMode >= 0
						|| calcDragMode(event.getClientX(), event.getClientY()) >= 0) {
	...
	protected int calcDragMode(int clientX, int clientY) {
		com.google.gwt.dom.client.Element resize = this.getCellElement(2, 2)
				.getParentElement();
	...
	protected void beginDragging(MouseDownEvent event) {
		...
		if ((this.resizable || this.draggable) && !this.minimized)
			dm = this.calcDragMode(event.getClientX(), event.getClientY());

		if (this.draggable && dm == 9) {
			super.beginDragging(event);
		} else if (this.resizable && dm >= 0) {
			this.dragMode = dm;

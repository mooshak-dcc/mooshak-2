...
public class ListingPresenter implements Presenter, ListingView.Presenter {
	...
	@Override
	public void onInfoSubmissionClicked(final String id, final String label,
			String problemId) {
		eventBus.fireEvent(new InfoSubmissionEvent(id, problemId));
	}

	@Override
	public void onDetailQuestionClicked(final String id, final String label) {
		eventBus.fireEvent(new DetailQuestionEvent(id));
	}
	...
	public void onSelectedItemChanged() {
		...
		if (selected != null) {
			...
			switch (selectedKind) {
			case SUBMISSIONS:
				onInfoSubmissionClicked(id, "Info", selected.getValue("problemId"));
				break;
			case QUESTIONS:
				onDetailQuestionClicked(id, "Detail");
				break;
			case BALLOONS:

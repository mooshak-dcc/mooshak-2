...
import pt.up.fc.dcc.mooshak.client.gadgets.kora.KoraView;
import pt.up.fc.dcc.mooshak.client.gadgets.kora.KoraViewImpl;
...
public class Kora extends Gadget implements HasLearningTime {
	...
	public Kora(HandlerManager eventBus, BasicCommandServiceAsync basicService,
			ParticipantCommandServiceAsync participantService, EnkiCommandServiceAsync enkiService,
			KoraCommandServiceAsync koraService, Token token, GadgetType type) {
		super(token, type);

		this.token = token;
		this.enkiService = enkiService;

		KoraView view = new KoraViewImpl();

		KoraPresenter presenter = new KoraPresenter(eventBus, basicService, participantService, enkiService,
				koraService, view, token);

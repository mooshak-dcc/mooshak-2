...
public class FilesDiff {
	 ...
	 */
	public FilesDiff(Object[] a, Object[] b) {
	...
	/**
	 * Prepare to find differences between two arrays. This ctor lets the caller
	 * computer equivalence numbers in any desired way. For instance, two char[]
	 * can be compared by copying to int[] with System.arraycopy().
	 */
	public FilesDiff(int[] a, int[] b) {
		for (int i : a)
			if (i >= equiv_max)
				equiv_max = i + 1;
		for (int i : b)
	...
	private void compareseq(int xoff, int xlim, int yoff, int ylim) {
		...
		if (xoff == xlim)
			...
			int c = cost;
			// int f = fdiag[fdiagoff + d]; // ZP not in use
